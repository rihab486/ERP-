 ...
 // const[serarchItem,setSearchItem]=useState('')
  function SelectSemaine(date1,date2,e) {
    ...
    if(loginemploye==false){ 
          fetch("http://127.0.0.1:8000/rapportsemaine/"+date1+"/"+date2+"/?id="+arr).then((result) => {
      ...
      fetch("http://127.0.0.1:8000/rapportsemaine/"+date1+"/"+date2+"/?id="+localStorage.getItem('id')).then((result) => {
        result.json().then((resp) => {
          setPost(true)
        setData(resp)
        setOpenn(false)
  ...
  <div className="table-responsive">
         ...
         <div className='card'>
               <div className='card-header' style={{backgroundColor:"#d1d0d0"}}>
                 <h3 style={{color:"black"}}>Importer Rapport de semaine de <strong>{date1}</strong> à<strong> {date2}</strong></h3>
               </div>
               <hr/>
           <div className="table-responsive" >
            <table  id="example">
