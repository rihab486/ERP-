  ...
  $(document).ready(function () {
    ...
    function SelectRapportAbsences(date1,date2,e) {
      ...
      if(loginemploye==false){ 
        ...
        fetch("http://127.0.0.1:8000/rapportabsence/" + date1 + "/" + date2 + "/?id=" + arr).then((result) => {
      ...
      fetch("http://127.0.0.1:8000/rapportabsence/" + date1 + "/" + date2 + "/?id=" + localStorage.getItem('id')).then((result) => {
        result.json().then((resp) => {
          setPost(true)
        setData(resp)
        setOpenn(false)
...
<input className="form-control" placeholder="" value={date2} name="date_pointage" onChange={(e) => setDate2(e.target.value)} type="date" />
    ...
    {loginemploye?   <div className="form-group pt-4"  style={{marginLeft:"45%",marginRight:"55%"}}>
                                        <a className="btn btn-primary"  style={{color:"white"}} data-dismiss="modal"  onClick={(e)=>{SelectRapportAbsences(date1,date2,e);;handleToggle()}}>Afficher</a></div>
...
<div className='col-md'>
    ...
    {openn?<Backdrop  open={openn}>
        <CircularProgress color="primary" style={{position:"absolute",bottom:"50px"}}  />
            ...
            <div className='card-header' style={{backgroundColor:"#d1d0d0"}}>
                  <h3 style={{color:"black"}}>Importer Rapport d'abscences de <strong>{date1}</strong> à<strong> {date2}</strong></h3>
                  </div>
                <hr/>
            <div className="table-responsive" >
       <table  id="example">
   ...
   {/** {post?    <div className="table-responsive"  >
    <ScrollContainer className="scroll-container">
